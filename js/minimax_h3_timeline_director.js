...
function normalizeState(raw) {
  ...
  base.audios = Array.isArray(raw.audios) ? raw.audios.filter(x => x?.file).slice(0, 3).map(a => ({ ...a, id:a.id || uid(), audioMode:a.audioMode==="locked"?"locked":"reference" })) : [];
...
class TimelineDirectorUI {
  ...
  async addFile(kind,file) {
    ...
    try{
      ...
      else { if(!info.hasAudio)throw new Error(tr("noAudioTrack")); this.state.audios.push({id:uid(),file:info.filename,name:file.name,duration:info.duration||0,trimStart:0,audioMode:"reference"}); }
